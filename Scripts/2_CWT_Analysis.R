...
### Includes post-processing analysis of Wavelet Results

### Corresponds to Figure 5

### Following Wavelets_for_fDOM_final_plots_lr_DWH.R
### A Hounshell, 23 December 2021
...
## Format for wavelet analysis: conduct wavelet analysis on half-hourly data
co2_data <- eddy_flux %>% 
...
png('./Fig_output/FigS7_Wavelet_both_halfhour.jpg',width=1100,height = 1300)
...
ggsave("./Fig_Output/Fig5_GlobalPower_All_halfhourly.png",width=9,height=7,units="in",dpi=320)
...
coutput_daily <- coutput %>% 
  ...
  dplyr::summarise(NEE = mean(NEE_uStar_f, na.rm = TRUE),
                   coutput_day = mean(coutput_day),
                   coutput_diel = mean(coutput_diel)) %>% 
  mutate(day_sum = ifelse(coutput_day>=0.5,1,0),
         diel_sum = ifelse(coutput_diel>=0.5,1,0))

coutput_daily$Date <- as.POSIXct(paste(coutput_daily$Year, coutput_daily$Month, coutput_daily$Day, sep = '-'), "%Y-%m-%d", tz = 'EST')

coutput_sum <- coutput_daily %>% 
  ungroup() %>% 
  summarise(day_sum = sum(day_sum),
            diel_sum = sum(diel_sum))
# Day = 76 days
# Diel = 34 days
...
coutput_ch4_daily <- coutput_ch4 %>% 
  mutate(Year = year(DateTime), 
         Month = month(DateTime), 
         Day = day(DateTime), 
         Hour = hour(DateTime)) %>% 
  dplyr::group_by(Year, Month, Day) %>% 
  dplyr::summarise(ch4 = mean(ch4_flux_uStar_f, na.rm = TRUE),
                   coutput_day = mean(coutput_day_ch4),
                   coutput_diel = mean(coutput_diel_ch4)) %>% 
  mutate(day_sum = ifelse(coutput_day>=0.5,1,0),
         diel_sum = ifelse(coutput_diel>=0.5,1,0))

coutput_ch4_daily$Date <- as.POSIXct(paste(coutput_ch4_daily$Year, coutput_ch4_daily$Month, coutput_ch4_daily$Day, sep = '-'), "%Y-%m-%d", tz = 'EST')

coutput_ch4_sum <- coutput_ch4_daily %>% 
  ungroup() %>% 
  summarise(day_sum = sum(day_sum),
            diel_sum = sum(diel_sum))
# Day = 82 days
...
day_sig <- ggplot()+
  ...
  geom_point(coutput_daily,mapping=aes(x=Date,y=day_sum,color="co2"),alpha=0.5)+
  geom_point(coutput_ch4_daily,mapping=aes(x=Date,y=day_sum+0.1,color="ch4"),alpha=0.5)+
...
diel_sig <- ggplot()+
  annotate("text",x=as.POSIXct("2021-04-05"),y=1.05,label="Sig.",size=5)+
  annotate("text",x=as.POSIXct("2021-04-01"),y=0.2,label="Not Sig.",size=5)+
  geom_point(coutput_daily,mapping=aes(x=Date,y=diel_sum,color="co2"),alpha=0.5)+
  geom_point(coutput_ch4_daily,mapping=aes(x=Date,y=diel_sum+0.1,color="ch4"),alpha=0.5)+
